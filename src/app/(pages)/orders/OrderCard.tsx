...
const OrderCard = ({ order }: { order: Order }) => {
  return (
    <div
      className="border p-4 rpunded-md gap-5 shadow items-center justify-between
    grid grid-cols-3 xl:grid-cols-5 hover:bg-zinc-100/10 transition cursor-pointer
    hover:shadow-lg"
    >
      <Image
        width={200}
      ...
      <p className="flex max-md:flex-col gap-1">
        ...
        <span className="font-bold text-red-600">{order.type}</span>
      </p>
    </div>
  );
};
